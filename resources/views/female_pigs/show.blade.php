            ...
            <div class="flex flex-col items-center gap-4 md:gap-6">
                ...
                <div class="flex max-w-md text-gray-600 lg:text-lg text-center">
                    ...
                    @if ($born_infos)
                        ...
                        <span class="text-red-600 text-base font-medium inline-flex items-center px-2.5 rounded">
                        @else
                        <span class="text-gray-800 text-base font-medium inline-flex items-center px-2.5 rounded">
                        @endif
                ...
                <!-- flag - start -->
                <div class="flex flex-row text-center mx-2">
                    <div class="w-full MplusRound inline-flex rounded-md" role="group">
                        <p class="py-1 pl-5 pr-4 text-xs font-medium">
                            良&emsp;好
                        </p>
                        <p class="py-1 px-4 text-xs font-medium">
                            不&emsp;調
                        </p>
                        <p class="py-1 pl-3 pr-2 text-xs font-medium">
                            再発注意
                        </p>
                        <p class="py-1 pl-2 pr-4 text-xs font-medium">
                            廃用候補
                        </p>
                    </div>
                </div>
                <div class="flex flex-row text-center mx-2">
                    ...
                    <div class="inline-flex rounded-md shadow-sm" role="group">
                        <form action="{{ route('female_pigs.updateFlag', $femalePig) }}" method="POST"
                            class="pt-1 px-6 text-sm rounded-l-lg transition-colors bg-gray-50 border active:bg-amber-500 font-medium border-gray-200 hover:text-white text-amber-300 hover:border-amber-300 hover:bg-amber-400 disabled:opacity-50">
                            @csrf
                            @method('PATCH')
                            <input type="hidden" name="well_flag" id=""
                                value="{{ $femalePig->well_flag == 0 ? 1 : 0 }}">
                            <button type="submit">
                                @if ($femalePig->well_flag == 0)
                                <div class="text-gray-200 hover:text-amber-300">
                                @else
                        ...
                        <form action="{{ route('female_pigs.updateFlag', $femalePig) }}" method="POST"
                            class="pt-1 px-6 text-sm transition-colors bg-gray-50 border active:bg-cyan-800 font-medium border-gray-200 hover:text-white text-cyan-600 hover:border-cyan-700 hover:bg-cyan-700 disabled:opacity-50">
                            @csrf
                            @method('PATCH')
                            <input type="hidden" name="unwell_flag" id=""
                                value="{{ $femalePig->unwell_flag == 0 ? 1 : 0 }}">
                            <button type="submit">
                                @if ($femalePig->unwell_flag == 0)
                                <div class="text-gray-200 hover:text-cyan-300">
                                @else
                                <div class="text-cyan-600 hover:text-cyan-300">
                                @endif
                                    <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor"
                                        class="w-5 h-5">
                                        <path fill-rule="evenodd"
                        ...
                        <form action="{{ route('female_pigs.updateFlag', $femalePig) }}" method="POST"
                            class="pt-1 px-6 text-sm transition-colors bg-gray-50 border active:bg-red-800 font-medium border-gray-200 hover:text-white hover:border-red-600 hover:bg-red-700 disabled:opacity-50">
                            @csrf
                            @method('PATCH')
                            ...
                            @if ($femalePig->recurrence_flag == 0) onclick="if(!confirm('再発要注意')){return false};" @endif>
                                ...
                                <div class="text-gray-200 hover:text-red-300">
                                @else
                                <div class="text-red-600 hover:text-red-300">
                                @endif
                                    <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor"
                                        class="w-5 h-5">
                                        <path
                        ...
                        <form action="{{ route('female_pigs.updateFlag', $femalePig) }}" method="POST"
                            class="pt-1 px-6 text-sm rounded-r-lg transition-colors bg-gray-50 border active:bg-red-800 font-medium border-gray-200 hover:text-white text-red-600 hover:border-red-700 hover:bg-red-700 disabled:opacity-50">
                            @csrf
                            @method('PATCH')
                    ...
                    @else
                    <div class="inline-flex rounded-md shadow-sm" role="group">
                        <div class="py-1 px-6 text-sm rounded-l-lg bg-gray-50 border font-medium border-gray-200">
                            @if ($femalePig->well_flag == 0)
                        ...
                        <div class="py-1 px-6 text-sm bg-gray-50 border font-medium border-gray-200">
                            @if ($femalePig->unwell_flag == 0)
                            <div class="text-gray-200">
                            @else
                        ...
                        <div class="py-1 px-6 text-sm rounded-r-lg bg-gray-50 border font-medium border-gray-200">
                            @if ($femalePig->warn_flag == 0)
                            <div class="text-gray-200">
                            @else
                            <div class="text-red-600">
                ...
                <div class="flex">
                    ...
                    <div>
                        ...
                        @if (date('Y-m-d H:i:s', strtotime('+3 day')) > $mixInfos->last()->first_recurrence_schedule &&
                            ...
                            @auth
                                <form action="{{ route('female_pigs.updateRecurrence', $femalePig) }}" method="POST">
                                    @csrf
                                    @method('PATCH')
        ...
        <table class="table w-full text-sm text-left text-gray-500">
            ...
            <tbody class="border-t border-b">
                @foreach ($mix_ranking as $item)
                    <tr class="bg-white">
                        <th scope="row" class="py-3 px-4 lg:px-6 font-medium text-gray-900 whitespace-nowrap">
                        </th>
